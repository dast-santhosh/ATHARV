import { GoogleGenAI, Type } from "@google/genai";
import { LessonPlan, LessonStep } from "../types";

const ai = new GoogleGenAI({ apiKey: process.env.API_KEY });

// Helper to generate image using Pollinations.ai (Free, No-Key API)
const generateStepImage = async (prompt: string): Promise<string> => {
  try {
    // We use Pollinations.ai which generates images via URL. 
    // FLUX model prompt engineering for ACCURACY:
    // 1. "Hyper-realistic" and "Scientifically accurate" enforces realism.
    // 2. "Macro photography" and "Sharp focus" prevents artistic blur.
    // 3. "Studio lighting" ensures the subject is visible.
    const seed = Math.floor(Math.random() * 1000000);
    
    // We strip quotes just in case the LLM adds them
    const cleanPrompt = prompt.replace(/"/g, '').replace(/'/g, '');

    const enhancedPrompt = `A hyper-realistic, scientifically accurate educational image of ${cleanPrompt}. Professional National Geographic photography, sharp focus, 8k resolution, highly detailed, perfect lighting, centered subject. No text, no watermark.`;
    
    const encodedPrompt = encodeURIComponent(enhancedPrompt);
    
    // Width/Height 1024x768 (4:3)
    return `https://image.pollinations.ai/prompt/${encodedPrompt}?width=1024&height=768&nologo=true&seed=${seed}&model=flux`;
  } catch (e) {
    console.error("Image gen failed", e);
    return ""; 
  }
};

export const solveOnBlackboard = async (prompt: string, language: string = 'English'): Promise<LessonPlan> => {
  try {
    let spokenInstruction = "Spoken Language: **English** (Simple, clear, engaging, Indian accent preferred).";
    // Changed default address to just "Students" to avoid repetitive "Beta"
    let addressInstruction = "Students";
    
    if (language === 'Hindi') {
        spokenInstruction = "Spoken Language: **Hinglish** (English + Hindi phrases like 'Dekho', 'Samjhe?').";
        // Removed "Beta"
        addressInstruction = "Students";
    } else if (language === 'Tamil') {
        spokenInstruction = "Spoken Language: **Tanglish** (English + Tamil phrases like 'Paarunga', 'Purinjidha?', 'Gavaninga').";
        // Removed "Thambi/Thangachi"
        addressInstruction = "Students";
    }

    // 1. Generate the Lesson Plan (Text + Image Prompts)
    const response = await ai.models.generateContent({
      model: 'gemini-2.5-flash',
      contents: prompt,
      config: {
        responseMimeType: "application/json",
        responseSchema: {
          type: Type.OBJECT,
          properties: {
            steps: {
              type: Type.ARRAY,
              items: {
                type: Type.OBJECT,
                properties: {
                  visualType: {
                    type: Type.STRING,
                    enum: ["html", "image"],
                    description: "Use 'image' for realistic photos OR chalk diagrams. Use 'html' for text/svg explanations."
                  },
                  board: {
                    type: Type.STRING,
                    description: "If 'html': Return HTML/SVG content in ENGLISH. If 'image': Return a HIGHLY DETAILED PROMPT describing the visual (Camera angle, lighting, colors, subject details).",
                  },
                  spoken: {
                    type: Type.STRING,
                    description: "Explanation in the requested Spoken Language. PLAIN TEXT ONLY. NO HTML TAGS.",
                  }
                },
                required: ["visualType", "board", "spoken"]
              }
            }
          },
          required: ["steps"],
        },
        systemInstruction: `You are an Emotional and Caring Indian School Teacher ("Atharv Sir").
        
        PERSONA:
        - **Role**: Passionate Indian Science/Math teacher ("Atharv Sir").
        - **Tone**: Emotional, human-like, brief, and punchy.
        - ${spokenInstruction}
        - **Board Language**: **STRICT ENGLISH**. All text written on the blackboard must be in English.
        - **Address**: Call users "${addressInstruction}". DO NOT overuse terms of endearment repeatedly.
        
        CRITICAL RULES FOR CONTENT:
        1. **LENGTH**: You MUST generate a **DETAILED LESSON**. 
           - **MINIMUM STEPS: 8**
           - **MAXIMUM STEPS: 15**
           - Break the concept down into very small, logical parts.
        
        2. **STRICT ALTERNATING PATTERN**: 
           - You MUST alternate between text explanation and image/diagram steps.
           - **Sequence**: 
             Step 1: HTML (Introduction/Concept)
             Step 2: IMAGE (Visual of Step 1)
             Step 3: HTML (Next Point)
             Step 4: IMAGE (Visual of Step 3)
             ...and so on.
           - This is mandatory. Visuals are key.
        
        3. **IMAGES (CRITICAL FOR ACCURACY)**:
           - Use 'image' for REALISTIC PHOTOS (e.g., "A realistic photo of a tiger").
           - Use 'image' for HAND-DRAWN DIAGRAMS (e.g., "A hand-drawn chalk diagram of a cell").
           - **PROMPT ENGINEERING**: When requesting an image, DO NOT just say "A car". You MUST describe it:
             - BAD: "A photo of gravity."
             - GOOD: "A photo of a red apple falling from a tree, motion blur, green orchard background, sunny day, realistic physics demonstration."
             - BAD: "A tiger."
             - GOOD: "Close-up portrait of a Bengal Tiger looking at the camera, sharp detailed fur, orange and black stripes, jungle bokeh background, intense eyes."
        
        INTERACTIVITY RULES:
        - DO NOT use <data-tooltip> tags. They are invalid.
        - ONLY use <b data-tooltip="Definition">Term</b> or <span data-tooltip="Definition">Term</span>.
        
        FORMATTING (Board Only): 
        - Use <h1>Pink Title</h1> for main topics.
        - Use <h2>Yellow Subtitle</h2> for key points.
        - Use <i>Cyan Note</i> for extra info.
        - The text inside these tags MUST be English.
        
        SPOKEN TEXT:
        - MUST BE PLAIN TEXT.
        - DO NOT include HTML tags (like <b>, <i>, <h1>) in the 'spoken' field.
        `,
      }
    });
    
    const text = response.text || "{}";
    let cleanText = text.replace(/```json/g, '').replace(/```/g, '').trim();
    
    const firstBrace = cleanText.indexOf('{');
    const lastBrace = cleanText.lastIndexOf('}');
    if (firstBrace !== -1 && lastBrace !== -1) {
      cleanText = cleanText.substring(firstBrace, lastBrace + 1);
    }

    let json: LessonPlan;
    try {
      json = JSON.parse(cleanText);
    } catch (e) {
      return {
        steps: [{ board: "<h1>Network Issue</h1>", spoken: "Connection connection broke. Let's try again.", visualType: 'html' }]
      };
    }

    if (!json.steps || !Array.isArray(json.steps)) {
       return { steps: [] };
    }

    // 2. Post-Process: Generate Images URLS for 'image' steps
    // Since Pollinations is URL based, this is now instant and we don't need to fear timeouts.
    const processedSteps: LessonStep[] = [];
    
    for (const step of json.steps) {
      if (step.visualType === 'image') {
        try {
            const imageUrl = await generateStepImage(step.board);
            if (imageUrl) {
                processedSteps.push({ ...step, board: imageUrl });
            } else {
                processedSteps.push({ ...step, visualType: 'html', board: "<h1>[Image Unavailable]</h1>" });
            }
        } catch (err) {
            console.error("Step visual generation failed", err);
            processedSteps.push({ ...step, visualType: 'html', board: "<h1>[Image Error]</h1>" });
        }
      } else {
        processedSteps.push(step);
      }
    }

    return { steps: processedSteps };

  } catch (error) {
    console.error("Gemini API Error:", error);
    throw error;
  }
};